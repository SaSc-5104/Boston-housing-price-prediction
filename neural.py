...
class MLP(nn.Module):
    def __init__(self, input_dim = 13):
        super().__init__()
        self.network = nn.Sequential(nn.Linear(input_dim, 64), nn.ReLU(), nn.Linear(64,32), nn.ReLU(), nn.Linear(32,1))
        
    def forward(self, x):
        return self.network(x).squeeze(1)
    
def train_model(X_train, y_train, epochs = 200, lr = 1e-3, batch_size = 32, val_split = 0.1, random_state = 42):
